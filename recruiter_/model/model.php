...
<<<<<<< HEAD
    ...
    // Support both md5 (seeded/registered data) and password_hash (future)
...
function sendOutreach($recruiterId, $seekerId, $jobId, $message) {
    ...
    // 1. Insert into recruiteroutreach for tracking (sent/read/responded)
    $stmt = mysqli_prepare($conn,
    ...
    // 2. Build enriched body for messages inbox
    $body = $message;
    if ($jid) {
    ...
    // 3. Mirror into messages table (applicationid is nullable — use NULL)
...
function sendMessage($senderId, $recipientId, $body) {
    global $conn;
    // Direct message — goes only into messages table (no outreach tracking)
    $stmt = mysqli_prepare($conn,
        "INSERT INTO messages (senderid, recipientid, applicationid, body, isread)
         VALUES (?, ?, NULL, ?, 0)");
...
function getMessagesByRecruiter($recruiterId) {
    global $conn;
    // Get all direct messages sent by this recruiter (not via outreach)
    // i.e. messages where there is NO matching recruiteroutreach entry
    $stmt = mysqli_prepare($conn,
        "SELECT m.*,
